...
function NavMbrSetUp(MmbrCntnr,PrMmbr,WhatMenu,Wdth,Hght) //MH:changed_function
{
	...
	if(RcrsLvl==1){
		...
		c=FontLowColor;
		d=FontHighColor;
		LoCss=CssItemClassName; //MH:changed
		HiCss=CssHiClassName; //MH:changed
		}
	else 
		{
		a=LowSubBgColor;
		...
		c=FontSubLowColor;
		d=FontSubHighColor	
		LoCss=CssSubClassName; //MH:changed
		HiCss=CssHiSubClassName; //MH:changed
		}
...
function CreateMenuStructure(MName,NumberOf)//MH:modified

{
	...
	if(Exp4){	MmbrCntnr.InnerString='';
		for(i=1;i<NumberOf+1;i++){
			WMnu=MName+eval(i);
			MmbrCntnr.InnerString+="<div id='"+WMnu+"' style='position:absolute;'><\/div>"}//MH:modified
...
/*********************************************************************************************************************************************
MH:
the original versions of the scripts


*/

function orig_HiliteItem(P){
	if(Nav4){
...
function orig_CntnrSetUp(Wdth,Hght,NoOff){
	var x=RcrsLvl==1?BorderColor:BorderSubColor;
	this.FrstMbr=null;
	this.OrgLeft=this.OrgTop=0;
	if(x)this.bgColor=x;
...
function orig_MbrSetUp(MmbrCntnr,PrMmbr,WhatMenu,Wdth,Hght){
	var Location=RcrsLvl==1?FrstLoc:ScLoc;
	var MemVal=eval(WhatMenu+'[0]');
	var t,T,L,W,H,S;
	var a,b,c,d;
...
function orig_NavMbrSetUp(MmbrCntnr,PrMmbr,WhatMenu,Wdth,Hght){
	var a,b,c,d;
	if(RcrsLvl==1){
		a=LowBgColor;
		b=HighBgColor;
